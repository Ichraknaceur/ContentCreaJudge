...
.PHONY: run
run: check-uv install ## Run the local API server
	uv run -- uvicorn contentcreajudge.api.app:app --host 127.0.0.1 --port 8000 --reload

.PHONY: run-ui
run-ui: check-uv install ## Run the Streamlit client UI
	uv run -- streamlit run src/contentcreajudge/ui/app.py --server.address 127.0.0.1 --server.port 8501
