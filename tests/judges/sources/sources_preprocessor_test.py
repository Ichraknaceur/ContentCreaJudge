...
import contentcreajudge.preprocessing.sources_preprocessor as sources_preprocessor_module
...
def test_preprocess_sources_extracts_html_anchor() -> None:
    ...
    result = preprocess_sources_content(content)
...
def test_preprocess_sources_casts_scalar_rel_value_to_string(monkeypatch) -> None:
    class FakeAnchor:
        ...
        def find_previous(self, tags: list[str]):
            return None
    ...
    monkeypatch.setattr(sources_preprocessor_module, "BeautifulSoup", FakeSoup)

    result = preprocess_sources_content("<p>ignored by fake soup</p>")
